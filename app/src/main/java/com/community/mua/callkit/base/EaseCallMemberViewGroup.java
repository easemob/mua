...
/**
* Created by lijian on 2020/09/20
* <p>
* 自定义ViewGroup类，会根据子控件的宽度自动换行，
*/
public class EaseCallMemberViewGroup extends ViewGroup {
...
//           if(i >=0 && i < 7){
//               if(i == 4 || i == 5){
//                   setViewParams(getChildAt(i), itemWidth,lastheight);
//               }else{
...
//           }
   ...
   private void handleItemClickAction(View v, int index) {
       if (isFullScreenMode()) {
           ...
           // 仅当开启视频后才能被点击进入全屏
           if (v instanceof EaseCallMemberView && !((EaseCallMemberView)v).isVideoOff()) {
               fullScreen(v);
           }
       }
