...
public class EditTextUtils {
    ...
    public static String ellipsizeString(TextView textView, String str, String keyword, int width) {
        ...
        if (str.length() - index <= count - 3) {
            String end = str.substring(str.length() - count + 3);
            end = "..." + end.substring(3);
            return end;
        }
        //如果是在中部的话，首尾显示省略号
        ...
        if (index > 20) {
            String middle = str.substring(index - 13, index + keyword.length() + subCount);
    ...
    public static String ellipsizeStringEnd(TextView textView, String str) {
    ...
    public static SpannableStringBuilder highLightKeyword(Context context, String str, String keyword) {
        ...
        if (keyword.length() >= str.length()) {
            ...
            builder.setSpan(new ForegroundColorSpan(ContextCompat.getColor(context, R.color.colorPrimary)), 0, str.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            return builder;
        }
        ...
        SpannableStringBuilder builder = new SpannableStringBuilder(str);
        builder.setSpan(new ForegroundColorSpan(ContextCompat.getColor(context, R.color.colorPrimary)), str.indexOf(keyword), str.indexOf(keyword) + keyword.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        return builder;
    }
